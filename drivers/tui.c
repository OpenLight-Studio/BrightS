...
static void draw_text_on_bar(int x, int y, const char *text, brights_color_t fg, brights_color_t bg)
{
  ...
  int py = y * 16;
  while (*text) {
    brights_font_draw_char(px, py, *text,
      (uint32_t)(fg.r << 16 | fg.g << 8 | fg.b),
      (uint32_t)(bg.r << 16 | bg.g << 8 | bg.b));
    px += 8;
    ++text;
  }
}
...
void tui_clear(void)
{
  ...
  tui_draw_title_bar("BrightS v0.1.2.5", NULL);
